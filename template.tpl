...

{
  ...
  "description": "This is an unofficial Google Tag Manager GA4 template for the Facebook Pixel. Updated from `simo-ahava` template",
  "securityGroups": [],
  "categories": [
    "ADVERTISING",
    "ANALYTICS"
  ...
  "version": 1,
  "brand": {
    "displayName": "gtm-ga4-templates-mcharkin",
...
[
  ...
  },
  {
    "type": "RADIO",
    "name": "ecommerce",
    "displayName": "dataLayer ecomerce format",
    "radioItems": [
      {
        "value": "none",
        "displayValue": "None"
      ...
      {
        "value": "enhanced",
        "displayValue": "Enhanced Ecommerce dataLayer integration"
      },
      {
        "value": "ga4",
        "displayValue": "GA4 Ecommerce dataLayer integration"
      }
    ],
    "simpleValueType": true,
    "help": "If you check one of these, then the Facebook pixel will populate \u003cstrong\u003eEvent Name\u003c/strong\u003e and \u003cstrong\u003eObject Properties\u003c/strong\u003e automatically from the last \u003ca href\u003d\"https://developers.google.com/tag-manager/enhanced-ecommerce\"\u003eenhanced ecommerce \u003c/a\u003e or \u003ca href\u003d\"https://developers.google.com/tag-manager/ecommerce-ga4\"\u003e ecommerce ga4\u003c/a\u003e object pushed into the dataLayer array."
  },
  {
    "type": "RADIO",
    "name": "eventName",
...
function converGA4ItemsToFBObjProps(items) {  
  const contents = items == null ? [] : items.map( (item) => ({
    id: item.item_id,
    quantity: item.quantity,
    item_price: item.price
  }) );
  
  const value = (items == null) ? undefined : items.reduce((acc, cur) => {
  ...
  return fbObjectProps;
}

function ga4EventNameToFB(eventName) {
  switch ( eventName ) {
    ...
    default: 
      fail('Facebook Pixel: Most recently pushed "ecommerce" object must be one of types "view_item", "add_to_cart", "begin_checkout" or "purchase".');
      return null;
  }
}

function getGA4Items(eventName, ecommerce) {
  if (eventName == "purchase") {
    if (!ecommerce.purchase || !ecommerce.purchase.items || getType(ecommerce.purchase.items) !== 'array') {
      fail('Facebook pixel: Most recently pushed "ecommerce" purchase object did not have a valid "purchase.items" array.');
      return null;
    }
    return ecommerce.purchase.items;
  } else {
     if (!ecommerce.items || getType(ecommerce.items) !== 'array') {
       fail('Facebook pixel: Most recently pushed "ecommerce" object did not have a valid "items" array.');
       return null;
     }
...
let eventName, action, eecObjectProps, ga4ObjectProps;
log("ecommerce", data.ecommerce);

if (data.ecommerce == "enhanced") {  
...
// try initializing GA4 ecommerce (either this of EEC, can't have both)
else if (data.ecommerce == "ga4") {
  if (!ecommerce) return fail('Facebook Pixel: No valid "ecommerce" object found in dataLayer');
  
  const event = copyFromDataLayer('event', 2);
  log("event:", event);
  if (!event) return fail('Facebook Pixel: No valid "event" object found in dataLayer');
  
  eventName = ga4EventNameToFB(event);
  if (eventName == null) { return; }
      
  const ga4Items = getGA4Items(event, ecommerce);
  if (ga4Items == null) { return; }    
    
  ga4ObjectProps = converGA4ItemsToFBObjProps(ga4Items);
...
const finalObjectProps = mergeObj( mergeObj(eecObjectProps || {}, ga4ObjectProps || {}), mergedObjectProps);
...
- name: Enhanced Ecommerce ViewContent works
  code: "mockData.ecommerce = \"enhanced\";\nmockData.objectPropertyList = {};\n\n\
    mock('copyFromDataLayer', key => {\n  if (key === 'ecommerce') return {\n    currencyCode:\
...
- name: Enhanced Ecommerce Purchase works
  code: "mockData.ecommerce = \"enhanced\";\nmockEec.fb.num_items = 3;\nmockData.objectPropertyList\
    ...
    assertThat(index, 'trackSingle called incorrect number of times').isEqualTo(2);\n\
    assertApi('gtmOnSuccess').wasCalled();"
- name: GA4 integration fails with invalid object
  code: |-
    mockData.ecommerce = "ga4";
    ...
    assertApi('gtmOnFailure').wasCalled();
    assertApi('gtmOnSuccess').wasNotCalled();
- name: GA4 intgration fails with invalid event value
  code: |-
    mockData.ecommerce = "ga4";
    ...
    assertApi('gtmOnFailure').wasCalled();
    assertApi('gtmOnSuccess').wasNotCalled();
- name: GA4 integration fails with invalid ecomerce object
  code: |-
    mockData.ecommerce = "ga4";
...
- name: GA4 ViewContent works
  code: "mockData.ecommerce = \"ga4\";\nmockData.objectPropertyList = {};\n\nmock('copyFromDataLayer',\
    ...
    \    \n      assertThat(arguments[3], 'trackSingle called with incorrect event\
    \ parameters').isEqualTo(mockGA4.fb);\n      index += 1;\n    }\n  };\n});\n \
    \    \n// Call runCode to run the template's code.\nrunCode(mockData);\n\n// Verify\
    \ that the tag finished successfully.\nassertThat(index, 'trackSingle called incorrect\
    \ number of times').isEqualTo(2);\nassertApi('gtmOnSuccess').wasCalled();"
- name: GA4 AddToCart works
  code: "mockData.ecommerce = \"ga4\";\nmockData.objectPropertyList = {};\n\nmock('copyFromDataLayer',\
